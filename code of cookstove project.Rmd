...
```{r}
# Step 1. Box-cox transformation on y.
lm_ss = lm(log(MAXBID) ~ Improves_Health + Time_Money + Time_Money_Health+FEMALE + AGE + MARRIED + KNOWLUNCH_Dummy + WIFECOOK + JOINTDECISION + WOOD+TSF_PRIMARY+BUYWOOD_DUMMY+ GATHERWOOD_DUMMY+STOVES1+DATESURVEY+PARISH+ ASSET_TOTAL-1,na.action = na.omit, data=data_plus)
summary(lm_ss)
# Step 2. transformation on independent variables that are not significant.
